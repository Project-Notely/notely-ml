    ...
    for backend in backends:
        ...
        try:
            ...
            if result.success:
                ...
                speed_bonus = max(0, 100 - (processing_time * 10))  # Bonus for faster processing
                
                performance_score = (confidence_score * 0.6) + (word_count * 0.5) + (speed_bonus * 0.1)
                ...
                print(f"   ✅ {word_count} words, {confidence_score:.1f}% confidence, {processing_time:.2f}s")
...
def smart_backend_selection_demo():
    ...
    test_images = [f for f in data_path.iterdir() if f.suffix.lower() in image_extensions]
    ...
    for image_path in sorted(test_images):
        ...
        print(f"   🎯 Highest Confidence: {analysis['best_confidence']['backend'].upper()} ({analysis['best_confidence']['confidence']:.1f}%)")
        print(f"   📊 Most Words Detected: {analysis['most_words']['backend'].upper()} ({analysis['most_words']['word_count']} words)")
        print(f"   ⚡ Fastest Processing: {analysis['fastest']['backend'].upper()} ({analysis['fastest']['processing_time']:.2f}s)")
        
        # Create highlighted image with the best backend
        best_ocr_result = best["ocr_result"]
        output_filename = f"smart_selection_{image_path.stem}_best_{best['backend']}.png"
        ...
        print(f"\n📊 DETAILED COMPARISON:")
        print(f"{'Backend':<12} {'Score':<8} {'Confidence':<12} {'Words':<8} {'Time':<8} {'Quality'}")
        print("-" * 70)
        ...
        for result in sorted(results, key=lambda x: x['performance_score'], reverse=True):
            ...
            print(f"{result['backend'].upper():<12} {result['performance_score']:<8.1f} {result['confidence']:<12.1f} {result['word_count']:<8} {result['processing_time']:<8.2f} {quality}")
