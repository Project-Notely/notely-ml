...
def switch_backends_for_different_content():
    ...
    for backend, description in backends_to_try:
        ...
        # Switch backend
        if backend == "trocr":
            processor.switch_ocr_backend(backend, trocr_model_type="handwritten_base", device="cpu")
        else:
        ...
        if result.success:
            print(f"   ✅ {result.result.total_words} words, {result.result.average_confidence:.1f}% confidence")
        else:
