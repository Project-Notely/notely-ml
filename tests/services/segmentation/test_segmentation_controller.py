...
class TestSegmentDocument:
    ...
    async def test_segment_document_success(self, sample_segmentation_result):
        ...
        with patch('app.controllers.segmentation_controller.SegmentationService') as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.segment_document = AsyncMock(return_value=sample_segmentation_result)
    ...
    async def test_segment_document_service_exception(self):
        ...
        with patch('app.controllers.segmentation_controller.SegmentationService') as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.segment_document = AsyncMock(side_effect=Exception("Service crashed"))
    ...
    async def test_segment_document_file_handling(self):
        ...
        with patch('app.controllers.segmentation_controller.SegmentationService') as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.segment_document = AsyncMock(return_value=SegmentationResult(
                success=True,
                segments=[]
            ))
    ...
    async def test_segment_document_options(self):
        ...
        for options in test_cases:
            with patch('app.controllers.segmentation_controller.SegmentationService') as mock_service_class:
                ...
                assert call_args[1]['infer_table_structure'] == options['infer_table_structure']
...
class TestControllerIntegration:
    ...
    async def test_concurrent_controller_calls(self):
        ...
        mock_files = [
            MockUploadFile(f"test_{i}.txt", f"content_{i}".encode())
            for i in range(3)
        ]
...
class TestControllerErrorCases:
    ...
    async def test_segment_document_memory_error(self):
        ...
        with patch('app.controllers.segmentation_controller.SegmentationService') as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.segment_document = AsyncMock(side_effect=MemoryError("Out of memory"))
    ...
    async def test_segment_document_timeout_error(self):
        ...
        with patch('app.controllers.segmentation_controller.SegmentationService') as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.segment_document = AsyncMock(side_effect=TimeoutError("Processing timeout"))
