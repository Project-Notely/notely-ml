...
class TestSegmentationService:
    ...
    def test_get_partition_strategy(self, segmentation_service):
        ...
        assert (
            segmentation_service._get_partition_strategy("fast")
            == PartitionStrategy.FAST
        )
        assert (
            segmentation_service._get_partition_strategy("hi_res")
            == PartitionStrategy.HI_RES
        )
        assert (
            segmentation_service._get_partition_strategy("auto")
            == PartitionStrategy.AUTO
        )

        # Test invalid strategy defaults to hi_res
        assert (
            segmentation_service._get_partition_strategy("invalid")
            == PartitionStrategy.HI_RES
        )
    ...
    def test_map_element_type(self, segmentation_service):
        ...
        assert (
            segmentation_service._map_element_type("NarrativeText")
            == SegmentType.PARAGRAPH
        )
        assert segmentation_service._map_element_type("Table") == SegmentType.TABLE
        assert (
            segmentation_service._map_element_type("ListItem") == SegmentType.LIST_ITEM
        )
        ...
        assert (
            segmentation_service._map_element_type("FigureCaption")
            == SegmentType.CAPTION
        )
        assert segmentation_service._map_element_type("Address") == SegmentType.TEXT
        assert (
            segmentation_service._map_element_type("EmailAddress") == SegmentType.TEXT
        )
        assert (
            segmentation_service._map_element_type("UncategorizedText")
            == SegmentType.TEXT
        )
    ...
    def test_calculate_confidence(self, segmentation_service):
        ...
        # Test long text
        long_element = MockElement(
            "This is a very long text that should have high confidence"
        )
    ...
    def test_convert_elements_to_segments(
        self, segmentation_service, mock_unstructured_elements
    ):
        """Test converting unstructured elements to segments"""
        segments = segmentation_service._convert_elements_to_segments(
    ...
    async def test_segment_document_success(
        self,
        mock_partition,
    ...
    @pytest.mark.asyncio
    @patch("app.services.segmentation_service.partition")
    async def test_segment_document_exception(
        self, mock_partition, segmentation_service, sample_text_file
    ):
    ...
    @pytest.mark.asyncio
    async def test_segment_document_with_different_strategies(
        self, segmentation_service, sample_text_file, mock_unstructured_elements
    ):
        ...
        for strategy in strategies:
            with patch("app.services.segmentation_service.partition") as mock_partition:
                ...
                result = await segmentation_service.segment_document(
                    sample_text_file, strategy=strategy
                )
    ...
    @pytest.mark.asyncio
    async def test_segment_document_with_options(
        self, segmentation_service, sample_text_file, mock_unstructured_elements
    ):
        ...
        with patch("app.services.segmentation_service.partition") as mock_partition:
            ...
            result = await segmentation_service.segment_document(
                sample_text_file, extract_images=False, infer_table_structure=False
            )
...
class TestSegmentationServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_real_text_file_segmentation(
...
class TestSegmentationServicePerformance:
    ...
    async def test_concurrent_processing(self, segmentation_service):
        ...
        with patch("app.services.segmentation_service.partition") as mock_partition:
            ...
            tasks = [segmentation_service.segment_image(img) for img in test_images]
