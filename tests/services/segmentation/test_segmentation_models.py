...
class TestBoundingBox:
    ...
    def test_bounding_box_validation_wrong_types(self):
        ...
        with pytest.raises(ValidationError):
            BoundingBox(
                x=10.5, y=20, width=100, height=50
            )  # x should be int, not float
...
class TestDocumentSegment:
    ...
    def test_document_segment_without_bbox(self):
        ...
        segment = DocumentSegment(
            text="Sample text", segment_type=SegmentType.PARAGRAPH, confidence=0.85
        )
    ...
    def test_document_segment_confidence_validation(self):
        ...
        segment1 = DocumentSegment(
            text="Test", segment_type=SegmentType.TEXT, confidence=0.0
        )
        assert segment1.confidence == 0.0

        segment2 = DocumentSegment(
            text="Test", segment_type=SegmentType.TEXT, confidence=1.0
        )
        assert segment2.confidence == 1.0

        segment3 = DocumentSegment(
            text="Test", segment_type=SegmentType.TEXT, confidence=0.5
        )
        assert segment3.confidence == 0.5

        # Invalid confidence values
        with pytest.raises(ValidationError):
            DocumentSegment(text="Test", segment_type=SegmentType.TEXT, confidence=-0.1)

        with pytest.raises(ValidationError):
            DocumentSegment(text="Test", segment_type=SegmentType.TEXT, confidence=1.1)

    def test_document_segment_missing_required_fields(self):
        """Test validation with missing required fields"""
        with pytest.raises(ValidationError):
            DocumentSegment(
                segment_type=SegmentType.TEXT, confidence=0.5
            )  # Missing text

        with pytest.raises(ValidationError):
            DocumentSegment(text="Test", confidence=0.5)  # Missing segment_type

        with pytest.raises(ValidationError):
            DocumentSegment(
                text="Test", segment_type=SegmentType.TEXT
            )  # Missing confidence
...
class TestSegmentationResult:
    ...
    def test_valid_segmentation_result_failure(self):
        ...
        result = SegmentationResult(
            success=False, segments=[], error="Processing failed"
        )
    ...
    def test_segmentation_result_with_empty_segments(self):
        ...
        result = SegmentationResult(success=True, segments=[])
    ...
    def test_segmentation_result_segments_validation(self):
        ...
        result = SegmentationResult(success=True, segments=[valid_segment])
        assert len(result.segments) == 1

        # Invalid segments (wrong type)
        with pytest.raises(ValidationError):
            SegmentationResult(success=True, segments=["not a DocumentSegment"])
    ...
    def test_segmentation_result_statistics_types(self):
        ...
        for stats in statistics_cases:
            result = SegmentationResult(success=True, segments=[], statistics=stats)
            assert result.statistics == stats
...
class TestModelsIntegration:
    ...
    def test_models_json_round_trip(self, sample_segmentation_result):
        ...
        assert (
            result_restored.total_segments == sample_segmentation_result.total_segments
        )
    ...
    def test_models_with_complex_nested_data(self):
        ...
        assert segment_restored.metadata["analysis"]["languages_detected"] == [
            "en",
            "fr",
        ]
