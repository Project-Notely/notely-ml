...
def run_tests(test_type="all", verbose=False, coverage=False):
    ...
    if coverage:
        cmd.extend(
            [
                "--cov=app.services.segmentation_service",
    ...
    if test_type == "performance":
        ...
        cmd.extend(
            ["-m", "not performance and not stress and not limit and not benchmark"]
        )
...
def run_quick_smoke_tests():
    ...
    cmd = [
        ...
        str(
            test_dir
            / "test_segmentation_models.py::TestBoundingBox::test_valid_bounding_box"
        ),
        str(
            test_dir
            / "test_segmentation_service.py::TestSegmentationService::test_init"
        ),
        "-v",
    ]
...
def main():
    ...
    # Run tests
    success = run_tests(
        test_type=args.test_type, verbose=args.verbose, coverage=args.coverage
    )
