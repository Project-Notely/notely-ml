...
def test_ocr_backends_standalone():
                ...
                results.append(
                    {
                        ...
                        "avg_confidence": sum(
                            w.get("confidence", 0) for w in word_positions
                        )
                        / len(word_positions),
                        "sample_words": [
                            w.get("text", "") for w in word_positions[:10]
                        ],
                    }
                )
    ...
    if successful_results:
        ...
        for result in successful_results:
            ...
            print(
                f"{result['backend'].upper():<12} {result['word_count']:<8} {result['avg_confidence']:<10.1f} {sample_text}"
            )
    ...
    if successful_results:
        ...
        best_backend = next(
            r for r in successful_results if r["word_count"] == best_count
        )
        print(
            f"• Most words detected: {best_backend['backend'].upper()} ({best_count} words)"
        )

        best_conf = max(r["avg_confidence"] for r in successful_results)
        best_conf_backend = next(
            r for r in successful_results if r["avg_confidence"] == best_conf
        )
        print(
    ...
    print(
        "2. Create .env.local file in project root with: GEMINI_API_KEY=your_api_key_here"
    )
    print("3. Run: processor = GeminiProcessor(ocr_backend='trocr')")
    print(
        "4. This will use Gemini for accurate text extraction + TrOCR for precise positioning"
    )
...
def test_gemini_processor_with_backends():
    ...
    if successful_results:
        ...
        for result in successful_results:
            print(
                f"{result['backend'].upper():<12} {result['word_count']:<8} {result['confidence']:<12.1f} {result['description']}"
            )
...
def test_backend_switching():
    ...
    print(
        f"EasyOCR words: {result1.result.total_words if result1.success else 'Failed'}"
    )

    # Switch to TrOCR
    processor.switch_ocr_backend(
        "trocr", trocr_model_type="handwritten_base", device="cpu"
    )
    ...
    print(
        f"Tesseract words: {result3.result.total_words if result3.success else 'Failed'}"
    )
